...
--   StateFunc (f :: s -> (a,s)) >>= g = StateFunc $ \s0 -> 
--     let 
--         (a,s1) = f s0 
--         StateFunc h = g a 
--         (b,s2) = h s1 
...
sf >>= g = StateFunc $ \s0 -> 
    let 
        (a,s1) = runStateFunc sf s0 
        (b,s2) = runStateFunc (g a) s1
...
getTheStackOfDishes :: StateFunc (Stack Int) (Stack Int)
getTheStackOfDishes = StateFunc $ \stackOfDishes -> (stackOfDishes, stackOfDishes)
